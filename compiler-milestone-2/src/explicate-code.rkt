...
(define (explicate-tail [exp : ExpR] [g : CFG]) : TailC
  (match exp
    [(Let x e body) (explicate-assign e x (explicate-tail body g) g)]
...
(define (explicate-assign [exp : ExpR] [x : Symbol] [tail : TailC] [g : CFG]) : TailC
  (match exp
    [(Let y exp2 body) (explicate-assign exp2 y (explicate-assign body x tail g) g)]
...
(define (explicate-pred [exp : ExpR] [thn : TailC] [els : TailC] [g : CFG]) : TailC
  (match exp
    ...
    [(Let var val body) (explicate-assign val var (explicate-pred body thn els g) g)]))
...
(module+ test
  ...
  (check-equal?
   (explicate-code
    (Program (Info)
             (Let 'y (Let 'x.1 20 (Let 'x.2 22 '(+ x.1 x.2))) 'y)))
   (Program (Info)
            '((start ((assign x.1 20)
                      (assign x.2 22)
                      (assign y (+ x.1 x.2))
