...
(define (print-x86 [p : X86]) : String
  ...
  (format BOILER-PLATE
          (string-join print-block (third p))
          stack-size stack-size))

(define (print-block [b : (Pairof Symbol BlockX86)]) : String
  (format "~a:\n~a" (car b)
          (string-join
           (λ (i)
             (format "    ~a\n" (print-instr (cast i InstrX86)))) (list-tail (cdr b) 2))))

(define (print-instr [i : InstrX86]) : String
  (match i
    [(cons op args) (format "~a ~a" op (string-join-delimiter (map print-arg args) ", "))]))

(define (print-arg [a : (U ArgX86 Symbol)]) : String
  (match a
    [`(deref ,r ,i) (format "[~a~a~a]" r (if (negative? i) "" "+") i)]
    [(or (? symbol? v) `(int ,v) `(reg ,v)) (~a v)]))
